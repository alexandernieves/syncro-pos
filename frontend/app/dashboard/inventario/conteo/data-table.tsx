            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="hover:bg-transparent border-b">
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead key={header.id} className="text-muted-foreground font-semibold h-11 px-4 text-[10px] uppercase tracking-widest text-center">
            ...
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-32 text-center text-sm text-muted-foreground italic">
                  No se registran datos en esta sección.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
